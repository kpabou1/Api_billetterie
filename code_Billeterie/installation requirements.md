...
Route::get('/import-database', 'DatabaseController@showImportForm')->name('import.database.form');
Route::post('/import-database', 'DatabaseController@import')->name('import.database');

<!-- resources/views/import.blade.php -->
<form action="{{ route('import.database') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="form-group">
        <label for="database-file">Sélectionner un fichier SQL :</label>
        <input type="file" class="form-control-file" id="database-file" name="database-file" required>
    </div>
    <button type="submit" class="btn btn-primary">Importer la base de données</button>
</form>
...
class DatabaseController extends Controller
{
    ...
    {
        $request->validate([
            'database-file' => 'required|mimes:sql|max:2048', // Validation du fichier SQL
        ]);

        $file = $request->file('database-file');

        $path = $file->storeAs('temp', 'import.sql'); // Stocker le fichier dans un répertoire temporaire

        // Exécutez la commande psql pour importer le fichier SQL
        $username = config('database.connections.pgsql.username');
        $database = config('database.connections.pgsql.database');
        $password = config('database.connections.pgsql.password');
        $command = sprintf('psql -U %s -d %s -f %s', $username, $database, storage_path('app/' . $path));
        ...
        return redirect()->back()->with('success', 'Base de données importée avec succès.');
    }
}
...
----------
Route::get('/export-database', 'DatabaseController@export')->name('export.database');
...
class DatabaseController extends Controller
{
    public function export()
    {
        ...
        exec("pg_dump -U {$username} -d {$database} > {$filename}");
...
<form action="{{ route('export.database') }}" method="GET">
    @csrf
    <button type="submit" class="btn btn-primary">Exporter la base de données</button>
</form>
