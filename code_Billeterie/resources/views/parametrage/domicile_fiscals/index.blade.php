...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">{{__('Liste des Domiciles Fiscaux')}}</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboards</a></li>
                        <li class="breadcrumb-item active">Domiciles Fiscaux liste</li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer d-flex justify-content-between">
                    <button type="button" class="btn btn-success" onclick="window.location='{{ route('domicile_fiscals.export', ['format' => 'xlsx']) }}'">
    ...
    <div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('domicile_fiscals.import') }}" method="post" enctype="multipart/form-data">
                        @csrf
    ...
    <br>
    <a href="{{ route('domicile_fiscals.create') }}" class="btn btn-primary btn-sm float-right">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered" id="domicileFiscalsTable">
            <thead class="table-light">
                <tr>
                    <th scope="col">#</th>
                    <th scope="col">Code</th>
                    <th scope="col">Domicile Fiscal</th>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            table = $('#domicileFiscalsTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('domicile_fiscals.index') }}",
                columns: [
                    { data: 'id', name: 'id' },
                    { data: 'code', name: 'code' },
                    ...
                    { data: 'created_at', name: 'created_at' },
                    {
                        ...
                        render: function(data, type, row) {
                            return `
                                <a href="{{ route('domicile_fiscals.show', ['id' => '__id__']) }}" class="btn btn-info btn-sm">
                                    <i class="bx bxs-bullseye"></i>
                                </a>
                                <a href="{{ route('domicile_fiscals.edit', ['id' => '__id__']) }}" class="btn btn-warning btn-sm">
                                    <i class="bx bxs-edit"></i>
                                </a>
                                <form action="{{ route('domicile_fiscals.destroy', ['id' => '__id__']) }}" method="POST" style="display:inline">
                                    @csrf
                                    @method('DELETE')
            ...
            $('#filterBtn').on('click', function() {
                ...
                table.ajax.url("{{ route('domicile_fiscals.index') }}?from_date=" + startDate + "&to_date=" + endDate).load();
                $('#filterModal').modal('hide');
            });

            // Gestion de la boîte de dialogue de confirmation avant la suppression
            $('#domicileFiscalsTable').on('click', '.delete-btn', function() {
                var form = $(this).closest('form');
                var domicileFiscalId = form.find('input[name="id"]').val();

                Swal.fire({
                    title: 'Confirmation',
                    text: 'Êtes-vous sûr de vouloir supprimer ce Domicile Fiscal ?',
                    icon: 'warning',
