...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">{{__('Liste des Formes Juridiques')}}</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboards</a></li>
                        <li class="breadcrumb-item active">Formes Juridiques liste</li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer d-flex justify-content-between">
                    <button type="button" class="btn btn-success" onclick="window.location='{{ route('forme_juridiques.export', ['format' => 'xlsx']) }}'">
    ...
    <div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('forme_juridiques.import') }}" method="post" enctype="multipart/form-data">
                        @csrf
    ...
    <br>
    <a href="{{ route('forme_juridiques.create') }}" class="btn btn-primary btn-sm float-right">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered" id="formeJuridiqueTable">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            table = $('#formeJuridiqueTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('forme_juridiques.index') }}",
                columns: [
                    { data: 'id', name: 'id' },
                    { data: 'code', name: 'code' },
                    ...
                    { data: 'created_at', name: 'created_at' },
                    {
                        ...
                        render: function(data, type, row) {
                            return `
                                <a href="{{ route('forme_juridiques.show', ['id' => '__id__']) }}" class="btn btn-info btn-sm">
                                    <i class="bx bxs-bullseye"></i>
                                </a>
                                <a href="{{ route('forme_juridiques.edit', ['id' => '__id__']) }}" class="btn btn-warning btn-sm">
                                    <i class="bx bxs-edit"></i>
                                </a>
                                <form action="{{ route('forme_juridiques.destroy', ['id' => '__id__']) }}" method="POST" style="display:inline">
                                    @csrf
                                    @method('DELETE')
            ...
            $('#filterBtn').on('click', function() {
                ...
                table.ajax.url("{{ route('forme_juridiques.index') }}?from_date=" + startDate + "&to_date=" + endDate).load();
                $('#filterModal').modal('hide');
            });

            // Gestion de la boîte de dialogue de confirmation avant la suppression
            $('#formeJuridiqueTable').on('click', '.delete-btn', function() {
                var form = $(this).closest('form');
                var formeJuridiqueId = form.find('input[name="id"]').val();

                Swal.fire({
                    title: 'Confirmation',
                    text: 'Êtes-vous sûr de vouloir supprimer cette forme juridique ?',
                    icon: 'warning',
